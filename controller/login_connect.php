        ...
        if ($stmt_user->rowCount() == 1) {
            ...
            if ($row_user['status'] == 'approved') {
                ...
                if (password_verify($password, $row_user['password'])) {
                    ...
                    if ($row_user['plan'] == 'basic') {
                        ...
                        $error_messages['plan'] = "Unrecognized user role.";
                    }
                } else {
                    // Incorrect password for user
                    $error_messages['password'] = "Invalid password for user.";
                }
            } else {
                // User status is not approved
                $error_messages['status'] = "User status is pending. Approval required.";
            }
        } else {
            ...
            if ($stmt_admin->rowCount() == 1) {
                ...
                if (password_verify($password, $row_admin['password'])) {
                    ...
                    $error_messages['password'] = "Invalid password for admin.";
                }
            } else {
                // Username not found in admin table
                $error_messages['email'] = "Username not found in users or admin table.";
            }
        }
