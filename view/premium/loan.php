     ...
     <?php
            ...
            if (isset($_SESSION['user_id'])) {
                ...
                if ($result->num_rows > 0) {
                    echo "<table border='1'>";
                    echo "<tr><th>No.</th><th>Transaction ID</th><th>Loan Amount</th><th>Date</th><th>Status</th><th>Note</th></tr>";
                    $count = 1;
                    while ($row = $result->fetch_assoc()) {
                        $date = new DateTime($row['date']);
                        $formattedDate = $date->format('m/d/y');
                        
                        echo "<tr><td>" . $count . "</td><td>" . htmlspecialchars($row['transaction_id']) . "</td><td>$" . number_format($row['loan_amount'], 2) . "</td><td>" . htmlspecialchars($formattedDate) . "</td><td>" . htmlspecialchars($row['status']) . "</td><td>" . htmlspecialchars($row['note']) . "</td></tr>";
